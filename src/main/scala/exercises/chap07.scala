...
object Par {
  ...
  def myMap2[A, B, C](a: Par[A], b: Par[B])(f: (A, B) => C): Par[C] = // `map2` doesn't evaluate the call to `f` in a separate logical thread, in accord with our design choice of having `fork` be the sole function in the API for controlling parallelism. We can always do `fork(map2(a,b)(f))` if we want the evaluation of `f` to occur in a separate thread.
    (es: ExecutorService) => {
      val af = a(es)
      val bf = b(es)
      UnitFuture(f(af.get, bf.get)) // This implementation of `map2` does _not_ respect timeouts, and eagerly waits for the returned futures. This means that even if you have passed in "forked" arguments, using this map2 on them will make them wait. It simply passes the `ExecutorService` on to both `Par` values, waits for the results of the Futures `af` and `bf`, applies `f` to them, and wraps them in a `UnitFuture`. In order to respect timeouts, we'd need a new `Future` implementation that records the amount of time spent evaluating `af`, then subtracts that time from the available time allocated for evaluating `bf`.
...
object MyApp {
  ...
  def sequence[A](ps: List[Par[A]]): Par[List[A]] = ps match {
    case Nil    => unit(Nil)
    case h :: t => map2[A, List[A], List[A]](h, sequence(t))(_ :: _)
  }

  def chooser[K, V](k: Par[K])(choices: K => Par[V]): Par[V] =
  ...
  def main9(args: Array[String]) {
    // Constructing new Callable is a blocking operation.
    // When the thread pool is out of threads,
    // Constructing new Callable will not exit.
  }

  def main8(args: Array[String]) {
    // see the after mentioned deadlock
  }

  def main7(args: Array[String]) {
    // map(y)(id) == y =>
    // unit(f(x)) = map(unit(f(x)))(id) = map(unit(x))(f)
    // uni(f(g(x))) = map(unit(f(g(x))))(id) = map(map(unit(g(x))))(f)
  }
